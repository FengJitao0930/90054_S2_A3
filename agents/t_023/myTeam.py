...
class myAgent(Agent):
    ...
    def avoid_must_lose_case(self, actions, game_state):
        ...
        for action in actions:
            y, x = action
            state_color_value = self.id
            if y == lose_point[0] or y == lose_point[1]:
                nextState = self.get_next_state(game_state, action, state_color_value)
                isSameColor = True
                for i in range(0, x):
                    if (nextState.board[i][y] != game_state.agent_colors[state_color_value]):
                        isSameColor = False
                        break
                nextStateScore = self.get_next_state_score(nextState, state_color_value)
                if isSameColor and current_max_score < nextStateScore:
                    current_max_score = nextStateScore
                    result = action
                isSameColor = True
                for i in range(x, lose_point[1]+1):
                    if (nextState.board[i][y] != game_state.agent_colors[state_color_value]):
                        isSameColor = False
                        break
                nextStateScore = self.get_next_state_score(nextState, state_color_value)
                if isSameColor and current_max_score < nextStateScore:
                    current_max_score = nextStateScore
                    result = action
            if x == lose_point[0] or x == lose_point[1]:
                nextState = self.get_next_state(game_state, action, state_color_value)
                isSameColor = True
                for i in range(0, y):
                    if (nextState.board[x][i] != game_state.agent_colors[state_color_value]):
                        isSameColor = False
                        break
                nextStateScore = self.get_next_state_score(nextState, state_color_value)
                ...
                isSameColor = True
                for i in range(y, lose_point[1] + 1):
                    if (nextState.board[x][i] != game_state.agent_colors[state_color_value]):
                        isSameColor = False
                        break
                nextStateScore = self.get_next_state_score(nextState, state_color_value)
        ...
        while self.time_consume(start_time) < 1 and not (len(Queue) == 0):
            ...
            for action in nextActions:
                ...
                for reverse_next_action in reverse_next_actions:
                    if self.time_consume(start_time) > 1:
                        break
                    next_action_State = self.get_next_state(nextState, action, 1 - self.id)
